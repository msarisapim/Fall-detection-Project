...
n=1;
        ...
        if(Rows>1)
            if(MeanPt>0)
                ...
                hold on;
                quiver(Y0, X0, -abs(double(largest_eigenvec(2)*sqrt(largest_eigenval))), -abs(double(largest_eigenvec(1)*sqrt(largest_eigenval))), '-r', 'LineWidth',3);
                quiver(Y0, X0, smallest_eigenvec(2)*sqrt(smallest_eigenval), smallest_eigenvec(1)*sqrt(smallest_eigenval), '-b', 'LineWidth',3);
                hold on;
                Frame=getframe(gcf);     
